...
def is_honeypot(token_address):
    ...
    try:
        ...
        if is_hp:
            ...
            return True, f"Honeypot confirmed: {reason}"
        if sell_tax > 15:
            honeypot_blacklist.add(token_address)
        ...
        return False, f"Clean — Buy tax: {buy_tax:.1f}% Sell tax: {sell_tax:.1f}%"
    except Exception as e:
...
def _send_graduation_alert(token_address, info, pair, current_mcap, change_from_graduation, alert_type, dex_url):
    ...
    if concentration and concentration > 50:
        print(f"Skipping {name} — high wallet concentration")
        return
...
def main():
    ...
    send_telegram(
        ...
        "⚡ Smart scan timing:\n"
        "   • Every 30s — graduation watchlist + milestones\n"
        "   • Every 60s — Dexscreener new + trending\n"
        "   • Every 3min — gainers + Birdeye\n"
        "👥 Group access system active\n\n"
