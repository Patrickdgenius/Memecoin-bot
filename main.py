...
def fetch_pumpfun_tokens():
    pairs = []
    try:
        # Only fetch tokens that have graduated or are close to graduating
        ...
        for coin in coin_list:
            try:
                ...
                # Stricter Pump.fun filters
                if mcap < 50_000:
                    continue
                if mcap > MCAP_MAX:
                    continue
                if age_hours and age_hours < 0.5:
                    continue  # ignore tokens under 30 mins old
                if age_hours and age_hours > 24:
                    continue
                if reply_count < 5:
                    continue  # needs real community activity
        ...
        for interval in TRACK_INTERVALS:
            ...
            if minutes_since >= interval:
                ...
                if current_mcap > 0 and alert_mcap > 0:
                    ...
                    label = "1 hour" if interval == 60 else "4 hours" if interval == 240 else "24 hours"
                    message = (
    ...
    for pair in pairs:
        try:
            ...
            if age_hours and age_hours > 48:
                continue
            if age_hours and age_hours < 0.5:
                continue  # skip anything under 30 mins
            ...
            if source == "pumpfun":
                if graduated:
                    score += 3
                    reasons.append("🎓 Graduated from Pump.fun — strong signal")
                else:
                    score += 1
                    reasons.append("🚀 Listed on Pump.fun")
            ...
            if buys_1h >= 20:
                score += 1
                reasons.append("👥 Strong buyer activity (20+ buys/hr)")
            ...
            if age_hours and age_hours < 1:
                score += 2
                reasons.append("🆕 Very fresh token (<1hr)")
            ...
            # Skip medium AND high rug risk for Pump.fun
            if source == "pumpfun" and rug_score >= 2:
                continue
            # Skip high rug risk for Dexscreener
...
def main():
    print("🤖 Memecoin Scanner Bot v7 started...")
    send_telegram(
        "🤖 *Memecoin Scanner Bot v7 is now LIVE!*\n\n"
        "📡 *Sources:*\n"
        "✅ Dexscreener Trending\n"
        "✅ Dexscreener Search\n"
        "✅ Pump.fun (stricter filters)\n\n"
        "🆕 *What's new:*\n"
        "✅ CA in every alert — tap to copy\n"
        "✅ Pump.fun rug filter tightened\n"
        "✅ Min 30 mins token age\n"
        "✅ Graduation bonus for Pump.fun\n"
        "✅ Buyer activity check\n\n"
