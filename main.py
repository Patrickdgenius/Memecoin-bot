...
EARLY_MCAP_MIN = 5_000
EARLY_MCAP_MAX = 80_000
EARLY_VOLUME_MIN = 3_000
EARLY_VOLUME_MAX = 15_000
EARLY_LIQUIDITY_MIN = 1_000
EARLY_AGE_MIN_MINUTES = 2
...
# Tokens matching these keywords are skipped entirely
...
“”“Skip tokens that are in mayhem/chaos mode — usually manipulation”””
...
“””
Skip tokens using ‘based’ narrative — since Meteora launched,
these are mostly low quality or manipulated pumps
“””
combined = f”{name.lower()} {symbol.lower()} {(description or ‘’).lower()}”
...
detected_narratives = []
score = 0
# NOTE: “based” removed from bullish keywords — Meteora launch made this narrative
# unreliable and mostly manipulated. Tokens using it are now skipped entirely.
bullish = {
“ai”: (“🤖 AI”, “ai_narrative”),
...
# NOTE: removed “based” from keyword searches
try:
...
```
            # Apply content filters to all pumpfun coins
            if is_mayhem_token(name, symbol, description):
                continue
            if is_based_narrative(name, symbol, description):
                continue

            # Early detection pool
...
```
        ...
        # Content filters applied to all tokens
        if is_mayhem_token(token_name, token_symbol, description):
            continue
        if is_based_narrative(token_name, token_symbol, description):
            continue
